...
/* GET /api/payment/vnpay/ipn — server-to-server confirmation from VNPay.
   Idempotent. RspCode per VNPay spec: 00 success, 97 invalid signature,
   01 order not found, 04 wrong amount, 02 already confirmed.
   DB failure → HTTP 500 so VNPay retries; never answer "not found" for a
   DB outage (that would lose a paid order). */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (params.vnp_ResponseCode === "00") {
      ...
      if (ok) void notifyOrder(orderId, "vnpay_paid"); // first confirmer only (markPaid is idempotent)
    ...
    if (isDbError(err)) {
      return Response.json({ RspCode: "99", Message: "System busy, retry later" }, { status: 500 });
    }
    throw err;
  }
}
